...
void Gfx::LoadImage(char *fn1)
{
  ...
  bmp = new BBitmap(BRect(1,1,bw,bh), B_RGBA32); // we need it later when copying raw data to it and after that rendering to view
  data = new unsigned char[4*bw*bh]; // raw bitmap data container
  ...
  // copy raw data from "data" array to bmp so we can see picture on screen also before spraying over it
  bmp->SetBits((void*)data,bh*bw*4,0,B_RGBA32); 
  
  delete bmp_h1; // delete original picture data, as we don't need it anymore
}
...
{ 
  ...
  bmp->SetBits((void*)data,bw*bh*4,0,B_RGBA32);
}
