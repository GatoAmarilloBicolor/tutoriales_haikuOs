...
#include <math.h>
  ...
  void PitchShiftEffect::SetPitch(float value)
  {
    if (value < 10) value = 10;
    shift_factor = value;
  }

  void PitchShiftEffect::SetParameter(float value, int param_index)
  {
    SetPitch(value);
  }
  

  void PitchShiftEffect::ApplyToSound(float *data, int samples)
  {
    float *data_out = new float[samples];
    smbPitchShift(shift_factor / 700.0f, samples, 1024, 32, 44100, data, data_out);
    for (int n = 0; n < samples; n++) data[n] = data_out[n];
    delete [] data_out;
  }

  void PitchShiftEffect::ClearBuffers()
  {
  }
